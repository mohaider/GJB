...
public class ForkLiftControllerPlayer2 : MonoBehaviour {
	...
	void Update () {
		if (Input.GetButton("Player2ForkliftUp") && Input.GetButton ("Player2ForkliftDown")) {
			upPressed = downPressed = false;
			return;
		}
		
		if (Input.GetButton("Player2ForkliftUp")) {
		...
		else if (Input.GetButtonUp("Player2ForkliftUp")) {
			upPressed = downPressed = false;
		}
		
		if (allowedDownMovement && Input.GetButton("Player2ForkliftDown")) {
			if (!downPressed) {
				downPressed = true;
				liftMoveTimer = 0.0f;
				currentPos = this.transform.localPosition;
		...
		else if (Input.GetButtonUp ("Player2ForkliftDown")) {
			upPressed = downPressed = false;
		}
	}
